...
class WildTeleportationStrategy(override val rule: WildRule) : CustomTeleportationStrategy<WildRule>() {
    ...
    override fun getLocation(context: CommandContext<ServerCommandSource>, spe: ServerPlayerEntity): Location? {
        if (rule.allowedDimension.none { allowedDim -> allowedDim == spe.world.dimensionEntry.key.get().value.toString() }) {
        ...
        return Location(loc.x, loc.y, loc.z, loc.pitch, loc.yaw, spe.world.dimensionEntry.key.get().value.toString())
    }
