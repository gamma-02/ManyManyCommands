...
import ch.skyfy.manymanycommands.api.config.WarpGroup
...
fun addWarp(
    ...
    yaw: Float = spe.yaw,
    dimension: String = spe.world.dimensionKey.value.toString()
) {
    Configs.WARPS.serializableData.warps.find { it.name == warpName }?.let {
        spe.sendMessage(Text.literal("A Warp with the name $warpName already exist").setStyle(Style.EMPTY.withColor(Formatting.RED)))
        return
    }
    Configs.WARPS.updateIterable(WarpConfig::warps) { it.add(Warp(warpName, Location(x, y, z, pitch, yaw, dimension))) }

    if (Configs.WARPS.serializableData.shouldAddNewWarpToTheDefaultGroup) {
//        Configs.WARPS.updateIterable(WarpConfig::groups) { warpGroups ->
//            warpGroups.firstOrNull { it.name == "DEFAULT" }?.warps?.add(warpName)
//        }
        Configs.WARPS.serializableData.groups.firstOrNull { it.name == "DEFAULT" }?.let { warpGroup ->
            Configs.WARPS.updateIterableNested(WarpGroup::warps, warpGroup.warps) { warps ->
